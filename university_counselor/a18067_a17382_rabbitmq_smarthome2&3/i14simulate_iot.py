...
def parse_args():
    ...
    parser.add_argument("--process_num", type=int, default=0, help="process number")
...
class IOT_Wrapper(object):
    def __init__(self, arguments_parser):
        ...
        self.con = pika.BlockingConnection(
            pika.ConnectionParameters(host=ip, port=port, credentials=user_pwd)
        )
        ...
        self.msg_con = pika.BlockingConnection(
            pika.ConnectionParameters(
                host=message_ip,
                port=message_port,
                credentials=message_user_pwd,
                heartbeat=0,
            )
        )
